...
const db = new sqlite3.Database(join(__dirname, 'scanner.db'), (err) => {
  if (err) {
    ...
    // Ajouter la colonne font_size si elle n'existe pas
    db.run(`
      ALTER TABLE users 
    ...
    `, (err) => {
      if (err && !err.message.includes('duplicate column')) {
        console.error('Erreur lors de l\'ajout de la colonne font_size:', err);
      }
    });

    // Ajouter la colonne confirmed à la table scans si elle n'existe pas
    db.run(`
      ALTER TABLE scans 
    ...
    `, (err) => {
      if (err && !err.message.includes('duplicate column')) {
        console.error('Erreur lors de l\'ajout de la colonne confirmed:', err);
      }
    });
